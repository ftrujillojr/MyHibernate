...
import org.me.exceptions.RSAException;
import org.me.utils.SysUtils;
...
public final class RSA {
    ...
    public RSA(String dirName, String projectName, int numBits) throws RSAException {
        this.algorithm = "RSA";
        this.privateKey = this.getResourcePath(dirName, projectName, "private.key");
        this.publicKey =  this.getResourcePath(dirName, projectName, "public.key");
    ...
    public boolean areKeysPresent() {
        ...
        if (privateKeyFile.exists() && publicKeyFile.exists()) {
            if (RSA.debug) {
                System.out.println("DEBUG: Using keys from \n" + this.privateKey + "\n" + this.publicKey + "\n");
            }
...
//            byte[] decodedText = decoder.decode(encText);
    ...
    /**
     * Since I am using RSA in a WebApplication and storing keys in src/main/resources/----/filename.key, I needed to resolve path
     * using class loader.
     * 
     * String resourcePath = this.getResourcePath(Paths.get("src", "main", "resources").toString(), "WormholeWebApp", "public.key");
     * 
     * @param srcDir
     * @param projectDir
     * @param fileName
     * @return
     */
    private String getResourcePath(String srcDir, String projectDir, String fileName) {
        String srcPath = Paths.get(srcDir, projectDir, fileName).toString(); // src/main/resources/WormholeWebApp/public.key
        String resourcePath = Paths.get(projectDir, fileName).toString();    // WormholeWebApp/public.key
        ...
        if (url != null) {
            ...
            if (file.exists()) {
                srcPath = url.getPath(); //  /u/ftrujillo/PESOFT/git-learning/WormholeWebApi/target/WormholeWebApi/WEB-INF/classes/WormholeWebApp/public.key
            }
        } 
//        else {
//            File file = new File(srcPath);
//            if (file.exists() == false) {
//                String msg = "ERROR: getResourcePath() could not resolve\n";
//                msg += "     srcPath => " + srcPath + "\n";
